...
func ValidateSafeURL(targetURL string) error {
	...
	// Check explicit IP address strings first
	ip := net.ParseIP(host)
	if ip != nil {
		if isPrivateIP(ip) {
	...
	if len(ips) == 0 {
		return fmt.Errorf("host '%s' resolved to 0 IP addresses", host)
	}

	for _, resolvedIP := range ips {
		if isPrivateIP(resolvedIP) {
			if resolvedIP.IsLoopback() && allowLoopback {
				continue
			}
			return fmt.Errorf("%w: host '%s' resolves to internal IP %s", ErrSSRFForbiddenIP, host, resolvedIP.String())
		}
	}
...
// isPrivateIP checks if an IP belongs to loopback, private, or metadata networks.
